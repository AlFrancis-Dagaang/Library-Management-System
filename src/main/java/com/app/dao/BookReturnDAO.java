...
public class BookReturnDAO {
    ...
    public BookReturnDAO(DBConnection db) {
       this.db = db;
    }


    public BookReturnStatus createBookReturnStatus(BookReturnStatus bookReturnStatus) {
        String sql = "INSERT INTO book_return_status (transaction_id, return_type, return_date, book_condition," +
        ...
        try(Connection con = this.db.getConnection()){
            ...
            ps.setString(2, bookReturnStatus.getReturnType());
            ps.setDate(3, java.sql.Date.valueOf(bookReturnStatus.getReturnDate()));
            ps.setString(4, bookReturnStatus.getBookCondition());
            ps.setString(5, bookReturnStatus.getReturnTimeline());
            ps.setBigDecimal(6, bookReturnStatus.getPenaltyAmount());
            ps.setBigDecimal(7, bookReturnStatus.getRefundAmount());
            ps.setString(8, bookReturnStatus.getLibrarianNotes());
            ps.executeUpdate();
    ...
    public BookReturnStatus getBookReturnStatus(int returnId) {
        ...
        try(Connection con = this.db.getConnection()){
            ...
            if(rs.next()){
                ...
                return new BookReturnStatus(returnStatusId, transactionId, returnType, returnDate, bookCondition, returnTimeline, penaltyAmount, refundAmount, librarianNotes);
            }
        }catch (SQLException e){
